...
# Install shared library runtime Qt6 yang dibutuhkan aplikasi saat berjalan
RUN apt-get update && apt-get install -y \
    libqt6core6 \
    && rm -rf /var/lib/apt/lists/*
...
WORKDIR /app
COPY --from=builder /app/build/dds_gateway_example .

# Update dynamic linker cache & set environment path
ENV LD_LIBRARY_PATH=/usr/local/lib:$LD_LIBRARY_PATH

# Entrypoint aplikasi
ENTRYPOINT ["./dds_gateway_example"]
